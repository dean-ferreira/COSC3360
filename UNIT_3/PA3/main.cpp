...
void *SFE(void * void_ptr) {
    ...
    arg->codeLength = ceil(log2(1 / currentProb) + 1); // calculate the code length
    std::string cw {};
    for (int x = 0; x < arg->codeLength; x++) {
        cw += fBarInBinary[x]; // assemble the code word based on the code length
    }
    ...
    std::cout << "Symbol " << currentSymbol << ", Code: " << cw << std::endl;
    pthread_mutex_lock(arg->semaphore2);
